...
CzLVCWallet::CzLVCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zlvc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZLVCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZLVCSeed_deprecated()) {
                    LogPrintf("%s: Updated zLVC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zlvc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zlvc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzLVCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZLVCCount(nCountLastUsed);
    else if (!walletdb.ReadZLVCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzLVCWallet::Lock()
{
    seedMaster = 0;
}

void CzLVCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzLVCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZLVC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzLVCWallet::LoadMintPoolFromDB()
{
...
void CzLVCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzLVCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzLVCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zlvcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzLVCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZLVC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zlvcTracker which also adds to database
    pwalletMain->zlvcTracker->Add(dMint, true);
...
void CzLVCWallet::SeedToZLVC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzLVCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzLVCWallet::GenerateDeterministicZLVC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzLVCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzLVCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzLVCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
