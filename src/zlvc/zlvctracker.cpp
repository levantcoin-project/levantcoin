...
CzLVCTracker::CzLVCTracker(std::string strWalletFile)
{
...
CzLVCTracker::~CzLVCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzLVCTracker::Init()
{
...
bool CzLVCTracker::Archive(CMintMeta& meta)
{
...
bool CzLVCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzLVCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzLVCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzLVCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzLVCTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzLVCTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzLVCTracker::GetSerialHashes()
{
...
CAmount CzLVCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzLVCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzLVCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzLVCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzLVCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzLVCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzLVCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzLVCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzLVCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzLVCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzLVCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzLVCWallet* zLVCWallet)
{
    bool iszLVCWalletInitialized = (NULL != zLVCWallet);
    CMintMeta meta;
    ...
    if (! iszLVCWalletInitialized)
        zLVCWallet = new CzLVCWallet(strWalletFile);
    meta.isSeedCorrect = zLVCWallet->CheckSeed(dMint);
    if (! iszLVCWalletInitialized)
        delete zLVCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzLVCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLVCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzLVCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzLVCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzLVCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzLVCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzLVCWallet* zLVCWallet = new CzLVCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zLVCWallet);
        }
        delete zLVCWallet;
        LogPrint("zero", "%s: added %d dzlvc from DB\n", __func__, listDeterministicDB.size());
    }
