...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZLVCModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zlvcTracker->HasMintTx(hash))) {
            //zLVC stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZLVC;
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by Levantcoin Address
...
bool IsZLVCType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZLVC:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZLVC || type == TransactionRecord::MNReward) {
