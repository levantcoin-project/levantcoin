...
#include "ui_zlvccontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zlvcTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zlvcTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zLVC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zLVC seed used to mint this zLVC is not the same as currently hold in the wallet";
            else
