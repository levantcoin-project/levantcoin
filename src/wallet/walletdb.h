...
public:
    ...
    bool WriteZLVCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZLVCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZLVCSeed_deprecated(uint256& seed);
    bool EraseZLVCSeed();
    bool EraseZLVCSeed_deprecated();

    bool WriteZLVCCount(const uint32_t& nCount);
    bool ReadZLVCCount(uint32_t& nCount);
