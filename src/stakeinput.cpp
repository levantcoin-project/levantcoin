...
// The zLVC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zLVC is a hash of the serial
...
{
    //Create an output returning the zLVC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZLVCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zLVC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zLVC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZLVCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zLVC output", __func__);
...
{
    CzLVCTracker* zlvcTracker = pwallet->zlvcTracker.get();
    CMintMeta meta;
    if (!zlvcTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zlvcTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a LVC stake is the outpoint
