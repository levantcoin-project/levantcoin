...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
    if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zLVC stake block is less than allowed start height", __func__);

    if (CZPivStake* zLVC = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zLVC->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zLVC stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zLVC stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zLVC->GetValue()));
        if (nChecksum200 != zLVC->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zLVC->GetChecksum(), nChecksum200);
    } else {
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight)
{
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s: staked zLVC fails context checks", __func__);
    } else {
        ...
        CPivStake* lvcInput = new CPivStake();
        lvcInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(lvcInput);
    }
    ...
    if (!txin.IsZerocoinSpend() && nPreviousBlockHeight >= Params().Zerocoin_Block_Public_Spend_Enabled() - 1) { //Equivalent for zLVC is checked above in ContextualCheckZerocoinStake()
