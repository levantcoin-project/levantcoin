...
/**
 * Levantcoin RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZlvcSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzlvcseed(const UniValue& params, bool fHelp);
extern UniValue getzlvcseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzlvc(const UniValue& params, bool fHelp);
extern UniValue dzlvcstate(const UniValue& params, bool fHelp);
